...
const Withdraw = () => {
  ...
  const handleWithdraw = (e) => {
    ...
    // Simulate withdrawal process (replace with API call)
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-16 px-4 text-gray-800 dark:text-white">
      <div className="max-w-md mx-auto bg-white dark:bg-gray-800 p-8 rounded-lg shadow">
        <h1 className="text-3xl font-bold text-center mb-6">Withdraw to M-Pesa</h1>
        ...
        <form onSubmit={handleWithdraw} className="space-y-6">
          ...
          <div>
            <label className="block mb-2 font-semibold">Amount to Withdraw (KES)</label>
            <input
              type="number"
              required
              value={amount}
          ...
          >
            {status === 'processing' ? 'Processing...' : 'Request Withdrawal'}
          </button>

          {status === 'success' && (
            <p className="text-center text-green-600 font-medium mt-4">
              ✅ Withdrawal request submitted! Check your M-Pesa shortly.
            </p>
          )}
        </form>
      </div>
