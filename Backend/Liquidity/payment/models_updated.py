...
class CoinRental(models.Model):
    ...
    user = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name="coin_rentals")
    coin = models.CharField(max_length=3, choices=COIN_CHOICES)
    amount = models.DecimalField(max_digits=12, decimal_places=2)  # amount paid in KES for renting
    created_at = models.DateTimeField(default=timezone.now)
    status = models.CharField(max_length=20, default="completed")

    def __str__(self):
        return f"{self.user.email} - Rented {self.coin} for {self.amount} KES"
